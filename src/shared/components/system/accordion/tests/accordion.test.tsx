...
import { testA11y } from '@/shared/lib/testing/test-utils';
...
describe('given a single Accordion', () => {
  ...
  it('should have no accessibility violations in default state', async () => {
    const { container } = render(<AccordionTest type="single" onValueChange={handleValueChange} />)
...
function AccordionTest(props: React.ComponentProps<typeof Accordion>) {
  return (
    <Accordion data-testid="container" {...props}>
      {ITEMS.map((val) => (
        <AccordionItem value={val} key={val} data-testid={`item-${val.toLowerCase()}`}>
          {({ open }) => (
            <>
